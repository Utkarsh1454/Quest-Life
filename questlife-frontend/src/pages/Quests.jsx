  ...
  return (
    <div className="space-y-8 animate-fade-in max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-gradient-to-r from-quest-dark/80 via-quest-dark to-quest-dark/80 p-6 rounded-2xl border border-white/10 shadow-xl relative overflow-hidden">
        ...
        <div>
          <h1 className="text-4xl font-heading font-bold mb-1 text-transparent bg-clip-text bg-gradient-to-r from-white via-quest-secondary to-quest-accent">
            Hero Quest Board
          </h1>
          <p className="text-gray-400 text-sm">Complete active tasks to earn XP rewards, gold, and progress your level.</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 relative z-10">
          ...
          <div className="glass-card px-4 py-2.5 rounded-xl border border-quest-gold/40 text-xs font-semibold flex items-center gap-2.5 text-quest-gold shadow-[0_0_15px_rgba(245,158,11,0.2)] bg-quest-gold/10">
            ...
            <div>
              ...
              <span className="font-mono text-sm text-white font-extrabold tracking-widest">
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-quest-primary/20 hover:bg-quest-primary/30 border border-quest-primary/40 text-xs font-bold text-white flex items-center gap-2 transition-all active:scale-95 shadow-[0_0_15px_rgba(246,36,64,0.3)] cursor-pointer"
            title="Force refresh quests for today"
          >
            <RotateCw className={`w-4 h-4 text-quest-secondary ${isRefreshing ? 'animate-spin' : ''}`} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredQuests.length === 0 ? (
          <div className="col-span-full text-center py-12 glass-card rounded-2xl border border-white/10">
            <Sparkles className="w-12 h-12 text-quest-gold mx-auto mb-3 animate-bounce" />
            <h3 className="text-xl font-heading font-bold mb-1">No Active Quests</h3>
            <p className="text-gray-400 text-sm mb-4">Click "Refresh Quests" above to assign fresh tasks!</p>
            <button
              onClick={handleManualRefresh}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start mb-2 gap-2">
                      <h3 className={`font-heading font-bold ${isBoss ? 'text-2xl text-quest-danger' : 'text-xl'}`}>
                        {quest.title}
                      </h3>
                      <span className="bg-quest-gold/20 text-quest-gold px-3 py-1 rounded-lg border border-quest-gold/30 font-bold text-xs whitespace-nowrap shadow-[0_0_10px_rgba(245,158,11,0.2)]">
                        +{quest.xp} XP
                      </span>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-xs font-medium">
                        <span>Progress</span>
                        <span className={isClaimed || isComplete ? 'text-quest-success font-bold' : 'text-gray-300'}>
